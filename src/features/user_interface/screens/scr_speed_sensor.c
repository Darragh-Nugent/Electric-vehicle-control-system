...
static void add_data(lv_timer_t *t)
{
    ...
    if (scale != NULL)
    {
        if (value > scaleYMax)
        {
            int32_t difference = value - scaleYMax;

            // Move Y min just below the smallest value currently, and move y Max just above
            int32_t yMin = LV_MAX(0, scaleYMin + difference - OVERHEAD_GAP);
            int32_t yMax = value + OVERHEAD_GAP;

            lv_scale_set_range(scale, yMin, yMax);
            lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, yMin, yMax);

            scaleYMin = yMin;
            scaleYMax = yMax;
        }
        else if (value < scaleYMin)
        {

            int32_t difference = scaleYMin - value;
            int32_t yMin = LV_MAX(0, (scaleYMin - difference - OVERHEAD_GAP));
            int32_t yMax = scaleYMax - difference + OVERHEAD_GAP;
            lv_scale_set_range(scale, yMin, yMax);
            lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, yMin, yMax);
            scaleYMin = yMin;
            scaleYMax = yMax;
...
void scr_speed_sensor_init(void)
{
    ...
    lv_obj_t *label = create_label(s_screen, "Speed");
